...
@author: Jake Thomas
This script runs the letterform recognition task under uniform and ideal phosphene conditions
"""
...
def initialise_parameters():
    ...
    [x, y, x_spacing, y_spacing] = rescale_uniform(x_pixels, y_pixels, list(range(0,param)), list(range(0,param)))
...
def rescale_uniform(x_pixels, y_pixels, x_uniform, y_uniform):
  x_spacing = x_pixels/len(x_uniform)
  y_spacing = y_pixels/len(y_uniform)

  x_rescaled = []
  y_rescaled = []

  for x in range(len(x_uniform)):
    x_rescaled = np.append(x_rescaled, x_uniform[x]*x_pixels/len(x_uniform) + x_spacing/2)
  for y in range(len(y_uniform)):
    y_rescaled = np.append(y_rescaled, y_uniform[y]*y_pixels/len(y_uniform) + y_spacing/2)
